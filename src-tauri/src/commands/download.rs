...
use crate::types::{
    parse_resolution, AudioStream, BaseStream, HeaderItem, StreamInfo, SubtitleStream, VideoStream,
};
...
pub async fn parse_url(
    ...
    let mut args: Vec<String> = vec![
        ...
        "--write-meta-json".to_string(),
        format!("--tmp-dir={}", temp_dir.display()),
        format!("--save-name={}", parse_id),
        "--no-log".to_string(),
    ];
    ...
    // 解析 JSON 文件
    let stream_info = parse_meta_json(&json_path)?;
...
fn parse_meta_json(json_path: &std::path::Path) -> Result<StreamInfo, String> {
    if !json_path.exists() {
