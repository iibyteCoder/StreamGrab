...
export function validateUrl(url: string): UrlValidationResult {
  ...
  return {
    valid: true,
    type: "unknown",
    error: "无法识别的流类型，可能不是 M3U8/MPD/MSS 链接",
  };
}
...
export function validateProxyUrl(proxyUrl: string): {
  ...
  const hasProtocol = supportedProtocols.some((p) =>
  ...
  if (!hasProtocol) {
    return {
      valid: false,
      error: "代理地址必须以 http://, https://, socks4://, socks5:// 开头",
    };
  }
...
export function validateRegex(pattern: string): {
  ...
  try {
    ...
    return {
      valid: false,
      error: `无效的正则表达式: ${e instanceof Error ? e.message : "未知错误"}`,
    };
  }
}
 ...
 */
export function validateHexKey(
  key: string,
  expectedLength = 16,
  ...
  if (cleaned.length !== expectedLength * 2) {
    return {
      valid: false,
      error: `密钥长度必须为 ${expectedLength * 2} 个十六进制字符`,
    };
  }
 ...
 */
export function validateBase64(str: string): {
  valid: boolean;
  error?: string;
} {
